...
repository_path = '..\Downloads\Trem2_senescent_microglia-main\';
...
% load and sorting ABT processed cytof data loading 
ABT_data_path = [data_path,'ABT.xlsx'];
[~,~,ABT_data_raw] = xlsread(ABT_data_path);
[ABT_protiens_levels_mat,ABT_mouse_genotype_ind,ABT_unique_cell_types,ABT_unique_protiens] = sort_cytof_data(ABT_data_raw,'ABT');
...
set(gca,'xtick',1:32','xticklabels',old_mice_unique_protiens,'ytick',1:9,'yticklabels',old_mice_unique_cell_types,...
...
xlabels = {'BAM','CD11b-','CD11b- SnCs','Microglia SnCs'};
...
lgd = legend(plt,num2str([1:9]'),'Location','southeast');
title(lgd,'Mouse ID')
legend('boxoff')
ylim([0 2.75])
xlim([0.5 2.5])
...
for comp = 1:3
    ...
    hold on
    scatter(comp_gene_across_exp,TREM2_across_exp,[],exp_labels,'filled')
    h=lsline;
    h.LineWidth = 1.5;
    xlim([-2.5 2.5])
    ylim([-2.5 2.5])
    [r,p] = corr(comp_gene_across_exp,TREM2_across_exp);
...
chosen_cell_types = [1:4,6:7];

mean_Trem2_WT_mice_protiens_levels_mat = mean(Trem2_WT_mice_protiens_levels_mat(chosen_cell_types,:,:),3,'omitnan');
mean_Trem2_KO_mice_protiens_levels_mat = mean(Trem2_KO_mice_protiens_levels_mat(chosen_cell_types,:,:),3,'omitnan');

figure('units','normalized','position',[0.05 0.3 0.8 0.3])
subplot(1,2,1)
imagesc(mean_Trem2_WT_mice_protiens_levels_mat,[0 6])
set(gca,'xtick',1:32','xticklabels',Trem2_unique_protiens,'ytick',1:6,'yticklabels',Trem2_unique_cell_types(chosen_cell_types),...
    'TickLabelInterpreter', 'none','box','off')
xtickangle(90)
title('5xFAD Trem2^+^/^+')
...
for comp = 1:4
    ...
    subplot(1,4,comp)
    hold on
    scatter(comp_gene_across_exp,ApoE_across_exp,[],exp_labels,'filled')
    h=lsline;
    h.LineWidth = 1.5;
    xlim([-2.5 2.5])
    ylim([-2.5 2.5])
    [r,p] = corr(comp_gene_across_exp,ApoE_across_exp);
    text(-2.25,2.25,['r = ',num2str(round(r,3))])
    text(-2.25,2,['p = ',num2str(round(p,5))])
